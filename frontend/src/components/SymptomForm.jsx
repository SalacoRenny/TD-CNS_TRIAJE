...
const SymptomForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Verificar que tenemos un usuario válido antes de enviar
    ...
    const payload = {
      userId: userId, // ✅ Se usa el ID del usuario logueado convertido a string
  ...
  // Función para mostrar el código del usuario con mejor manejo de estados
  const getUserDisplayValue = () => {
    console.log("🔍 Evaluando usuario para display:", user);
    if (user === null) return "Cargando...";
    if (user === undefined) return "Error de sesión";
    
    // Verificar múltiples posibles propiedades del usuario
    const possibleIds = [
      user.id,
      user._id,
      user.userId,
      user.codigo,
      user.codigoAsegurado,
      user.username,
      user.email
    ];
    
    const foundId = possibleIds.find(id => id && id !== "");
    console.log("🎯 ID encontrado:", foundId);
    
    if (!foundId) return "Sin código asignado";
    return foundId;
  };

  // Función para obtener el ID del usuario para el payload
  const getUserId = () => {
    if (!user) return null;
    ...
    const foundId = possibleIds.find(id => id && id !== "");
    console.log("📤 ID para payload:", foundId);
    
    return foundId ? String(foundId) : null; // Convertir a string como espera el backend
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50 to-green-100 p-6 flex items-center justify-center">
      <div className="relative w-full max-w-4xl">
        ...
        >
          ...
          <div className="group">
            ...
            <div className="relative">
              <input
                ...
                disabled
                className={`w-full px-4 py-3 pl-12 rounded-xl border-2 border-gray-200 cursor-not-allowed text-gray-600 font-mono ${
                  getUserId() ? 'bg-gradient-to-br from-green-50 to-teal-50' : 'bg-gradient-to-br from-red-50 to-orange-50'
                }`}
              />
              <User className={`absolute top-3 left-3 ${
                getUserId() ? 'text-green-500' : 'text-orange-500'
              }`} size={20} />
            ...
            {/* Mensaje de ayuda si no hay usuario */}
            {!getUserId() && (
          ...
          <div className="pt-6">
            <button
              type="submit"
              disabled={isSubmitting || !getUserId()}
      ...
      {/* CSS corregido - SIN atributo jsx */}
      <style>{`
        @keyframes shimmer {
